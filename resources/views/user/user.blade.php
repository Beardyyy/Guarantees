        ...
        <section class="px-6 py-8">
            ...
            <div class="mb-6">
                <button type="submit" class="bg-blue-400 rounded text-white py-2 px-4 hover:bg-blue-500">
                    <a href="/user/{{ $user->id }}/password">Change password</a>
                </button>
            </div>

            <form action="/user/{{ $user->id }}" method="POST" enctype="multipart/form-data" class="max-w-lg mx-auto mt-10 bg-gray-100 p-6 rounded-xl border border-gray-200">
                @csrf
                @method('PATCH')
